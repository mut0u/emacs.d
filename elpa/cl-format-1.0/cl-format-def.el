...
following marker, are treated as parameters.
...
The different argument types have differnt value domains, as
follows.
...
&parameter are always integer, which contain the domain of
characters as a subset. Note, that the dynamic parameter \(# and
...
or end separator subdirectives.
...
Finally here is a complete example of a directive, implementing a
fill operation.  It uses the generalized ~CHAR(...~) form, an
  ...
  (let* ((char-beg (if (consp char)
    ...
    (let* ((parm-only (cdr (assq '&parameter args)))
           ...
           (let-args-2
            (mapcar (lambda (arg)
                      `(list ',(cdr arg)
                             (list 'quote ,(cdr (assq (car arg) ll)))))
                    non-parm-arg))
           (idx -1)
           (parm-symbol (make-symbol "parameter"))
           (parm-raw-symbol (make-symbol "parameter-raw"))
           (let-args-3
            (mapcar
             (lambda (a)
               (or (consp a)
                              ...
                              (append
                               (list
                                (list 'setq ',parm-symbol
                                      (list 'cdr ',parm-raw-symbol)
                                      ',args-sym (list 'car ',parm-raw-symbol))
                                (cons 'let
                                      (cons
                                       (list 
                                        ,let-args-1
